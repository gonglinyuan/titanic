...
def run(x: np.ndarray, y: np.ndarray, *, distribution: str, dev_type: str, hidden_units: int, iter_num: int,
        friction: float, learning_rate: float) -> (float, float):
    ...
    for cv_id in range(SUBSET_NUM):
        ...
        w, b = nN.init((x_train.shape[0], hidden_units, y_train.shape[0]), distribution=distribution, dev_type=dev_type)
...
def run_test(x_train: np.ndarray, y_train: np.ndarray, x_test, *, distribution: str, dev_type: str, hidden_units: int,
             iter_num: int, friction: float, learning_rate: float) -> np.ndarray:
...
def main():
    ...
    param_list = (("UNIFORM", "FAN_IN", 10, 1000, 0.1, 0.1),)
...
def test():
    ...
    y_test = run_test(x_train, y_train, x_test, distribution="UNIFORM", dev_type="FAN_IN", hidden_units=10,
